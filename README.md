...
### Execution Workflow:
1. Select the **Zlatý standard** reference file. The application defaults to `virtual_stream_ground_truth_complete_time.json`. *(Note: The compiled continuous audio track can be manually found at `AudioAnalysisTool\data\processed\virtual_stream_complete_time.wav`. If you are not using this compiled track and are instead evaluating individual standalone samples, you must switch the reference file to `virtual_stream_ground_truth.json`).*
